...
export class OrdenesPagoService {
  ...
  resumen(params: {
    ...
    from?: string;
    to?: string;
  }): Observable<Record<OrdenPagoEstado, number>> {
    const cleaned = Object.entries(params || {}).filter(([, v]) => v != null && v !== '');
    let httpParams = new HttpParams();
    cleaned.forEach(([k, v]) => (httpParams = httpParams.set(k, String(v))));
    return this.http.get<Record<OrdenPagoEstado, number>>(`${API}/OrdenesPago/resumen`, { params: httpParams });
  }
  ...
  generarParaMes(anio: number, mes: number) {
    const params = new HttpParams().set('anio', String(anio)).set('mes', String(mes));
    return this.http.post(`${API}/OrdenesPago/generar`, null, { params });
  }

  marcarPagada(id: number): Observable<OrdenPagoResumenDTO> {
    return this.http.post<OrdenPagoResumenDTO>(`${API}/OrdenesPago/${id}/pagar`, null);
  }
